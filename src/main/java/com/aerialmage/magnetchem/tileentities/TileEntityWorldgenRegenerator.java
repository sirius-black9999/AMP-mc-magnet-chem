    ...
    public void updateEntity()
    {
        ...
        if (!this.worldObj.isRemote)
        {
			...
			if(cooldown <= 0)
			{
				...
				if(accuValue > WorldgenMonitor.getItemRarity(new ItemStack(Block.getBlockById(selectedItemId)), worldObj, xCoord, zCoord))
				{
					smeltItem();
				}
			}
...
//            this.onInventoryChanged();
    ...
    {
    	furnaceItemStacks[0] = new ItemStack(Block.getBlockById(selectedItemId));
    	accuValue = 0;
    }
	...
	public void fillItemSlots() {
    	...
    	for(int i = 0; i < 20; i++)
		{
    		...
    		if(entry < entries.length)
    		{
				...
				setInventorySlotContents(i+1, new ItemStack(Item.getItemById(entries[entry])));
    		}
    		else
    		{
